...
class Demo : public EDAnalyzer {
private: 
  bool dumpLHE_;
  bool checkPDG_;
public:
  explicit Demo( const ParameterSet & cfg ) : 
    tokenLHERunInfo_(consumes<LHERunInfoProduct,edm::InRun>(cfg.getUntrackedParameter<edm::InputTag>("moduleLabel", std::string("externalLHEProducer")) ) ),
    tokenLHEEvent_(consumes<LHEEventProduct>(cfg.getUntrackedParameter<edm::InputTag>("moduleLabel", std::string("externalLHEProducer")) ) )
  {
  }
private:
...
//    }
	...
	if (it->tag() != "initrwgt" && it->tag()!="madspin" ) continue;
